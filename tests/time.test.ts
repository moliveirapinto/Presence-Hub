import {
  clampSpan, dayBounds, fmtClock, fmtShort, fmtTimeRange, getInitials, isToday, toUtcLiteral,
} from "../PresenceHub/time";
...
describe("isToday", () => {
  ...
  it("is false for the same day in another year", () => {
    expect(isToday(new Date(2025, 8, 17, 12, 0), now)).toBe(false);
  });
});

describe("fmtTimeRange", () => {
  it("uses the supplied label for an open-ended range", () => {
    const s = fmtTimeRange(new Date(2026, 8, 17, 9, 14).toISOString(), null, "en", "agora");
    expect(s).toContain("agora");
  });
});
...
describe("getInitials", () => {
  ...
  // Regression: a whitespace-only display name used to throw.
  it.each(["", "   ", null, undefined])("falls back to ? for %p", (name) => {
    expect(getInitials(name as unknown as string)).toBe("?");
  });
});
